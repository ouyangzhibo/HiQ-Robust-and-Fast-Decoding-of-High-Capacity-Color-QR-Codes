 ...
 *
 */
final class InsideAlignmentPatternFinder{
	...
	protected final ResultPointCallback resultPointCallback;
	   ...
	   */
	  InsideAlignmentPatternFinder(BitMatrix image,
	                         int startX,
	                         int startY,
	                         int width,
	                         int height,
	  ...
	  AlignmentPattern find() throws NotFoundException {
	    ...
	    // the ratio remains unchanged, but each of which is 2 times of the estimated module size
	   ...
	   *         but each component shoule be changed to 2 times of the module size
	   */
	  private boolean foundPatternCross(int[] stateCount) {
	    float moduleSize = this.moduleSize;
	    moduleSize = 2 * moduleSize;//add by elky
	    //make the maxVariance more strict for inside alignment pattern?
	  ...
	  private AlignmentPattern handlePossibleCenter(int[] stateCount, int i, int j) {
	    ...
	    if(!Float.isNaN(centerI) && !Float.isNaN(centerJ)) {
	      ...
	      //should be "/6.0f" since the size has been changed to 2 times as the original
	      float estimatedModuleSize = (float) (stateCount[0] + stateCount[1] + stateCount[2]) / 6.0f;
