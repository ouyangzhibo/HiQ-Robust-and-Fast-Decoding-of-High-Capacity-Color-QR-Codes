...
public final class QRCodeMultiReader extends QRCodeReader implements MultipleBarcodeReader {
    ...
    for (DetectorResult detectorResult : detectorResults) {
      try {
        ...
        Result result = new Result(decoderResult.getText(), decoderResult.getRawBytes(), points,
                                   BarcodeFormat.QR_CODE,decoderResult.getDataBlocks());
  ...
  private static List<Result> processStructuredAppend(List<Result> results) {
    ...
    for (Result result : results) {
      if(result.getIntMetadata(ResultMetadataType.STRUCTURED_APPEND_SEQUENCE) >0) {
        hasSA = true;
        break;
      }
    }
    ...
    for(Result saResult : saResults){
      ...
      Iterable<byte[]> byteSegments = 
    		  (Iterable<byte[]>) saResult.getByteMetadata(ResultMetadataType.BYTE_SEGMENTS);
      if(byteSegments !=null){
    ...
    Result newResult = new Result(concatedText.toString(), newRawBytes, NO_POINTS, BarcodeFormat.QR_CODE,null);
    if (byteSegmentLength > 0) {
      List<byte[]> byteSegmentList = new ArrayList<>();
  ...
  private static final class SAComparator implements Comparator<Result>, Serializable {
    ...
    public int compare(Result a, Result b) {
      ...
      int aNumber = (int) (a.getIntMetadata(ResultMetadataType.STRUCTURED_APPEND_SEQUENCE));
      int bNumber = (int) (b.getIntMetadata(ResultMetadataType.STRUCTURED_APPEND_SEQUENCE));
      if (aNumber < bNumber) {
        return -1;
      }
      if (aNumber > bNumber) {
