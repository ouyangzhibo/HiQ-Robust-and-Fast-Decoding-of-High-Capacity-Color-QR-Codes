...
/**
 * This class stores the private key, digital certificate and other variables/functions related 
 * to digital signature creation
...
//Part 2: saving and parsing the private key and digital certificate 	
	/**
	 * Given a PrivateKeyInfo or a PEMEncryptedKeyPair with password, extract the private key 
	 * and save it
	...
	public boolean savePrivateKey(PrivateKey key, String algorithm){
		...
		senderKey=key;
		signatureAlgorithm=null;
		if(algorithm.compareTo("EC")==0 || algorithm.compareTo("ECDSA")==0){
			try {
			...
			try{
				signatureAlgorithm=getAvailableSignAlgo(senderKey, "RSA");
				if(signatureAlgorithm!=null) return true;
				signatureAlgorithm=getAvailableSignAlgo(senderKey, "DSA");
				if(signatureAlgorithm!=null) return true;
			}catch(InvalidKeyException | NoSuchAlgorithmException e){ 
				Log("Error on getting the private key for signature : "+e.getMessage());
			}
	...
	private static String getAvailableSignAlgo(PrivateKey privateKey, String keyType) 
	...
	public boolean setCertificate(X509Certificate cert){
		senderCert=cert;
	...
	public boolean saveCertificate(List<X509CertificateHolder> certList){
		...
		//Here we did not verify the digital certificate, we only take the 
		//first certificate as the issuer certificate
		//TODO: verify the selected digital certificate (which may be a chain of certificate)
		//and make sure it matches the selected private key
		try {
			senderCert=new JcaX509CertificateConverter().setProvider("BC")
					.getCertificate(certList.get(0));
			return (senderCert !=null)? true:false;
...
//Part 3: giving an Autheenticated 2D barcode creation object 	
	public Auth2DbarcodeDecoder getBarcodeCreatorObj(boolean isTextOnly){
		if(getPrivateKey() ==null || getSignatureAlgorithm() ==null || getCertificate()==null) 
			return null;
		Auth2DbarcodeDecoder barcode = (!isTextOnly)? 
				new AuthBarcodeObjBSON(getPrivateKey(), getCertificate())   
				:new AuthBarcodePlainText(getPrivateKey(), getCertificate());
		try {
