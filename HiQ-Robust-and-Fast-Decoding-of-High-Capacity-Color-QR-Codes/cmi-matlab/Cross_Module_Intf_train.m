function [ ] = Cross_Module_Intf_train( )
...
elseif strcmp(modelString, 'quadratic')
    ...
    if istrain == 1
        ...
        for i = 1 : maxIter
            ...
            fprintf('index: %d deltaM = %f trainACC = %f testACC = %f objval= %f, deltaObj = %f\n',...
                    i, deltaM, acc, testacc, objval, deltaObj);
...
elseif strcmp(modelString, 'svm2') % extend existing SVM
    ...
    W15_all = zeros(featDim*5+1, 3);
    M_all = zeros(5, 3);
    
    W_all = [22.7200  -12.7230   -0.9572
          -12.5074   25.2888  -16.0901
           -1.3086   -0.7549   25.3609
            3.3319    3.4416    2.3543];


    M_all = [0.9959    0.9978    0.9977
       -0.0600   -0.0378   -0.0305
       -0.0451   -0.0380   -0.0387
       -0.0386   -0.0359   -0.0433
       -0.0329   -0.0172   -0.0194];
    ...
    if istrain == 1
        ...
        for layerIdx = 1:3
            ...
            optObj = 1e-4; % convergence threshold
            lambda = 1e-8; % learning rate
            lastM = [1,0,0,0,0]';
            M = lastM;
            lastObj = inf;
            [X_train_raw, Y_train] = extractRawFeature(data_train, train_gt_layer, mapping);
            trainNum = size(X_train_raw, 3);
            X_train = zeros(trainNum, featDim);
            
            [X_test_raw, Y_test] = extractRawFeature(data_test, test_gt_layer, mapping);
            testNum = size(X_test_raw, 3);
            X_test = zeros(testNum, featDim);
            
%             [acc15, W15_all(:,layerIdx)] = evaluateSVMFeat15 ( X_train_raw, Y_train, X_test_raw, Y_test);
%             fprintf('Layer %d: 15-dim feature accuracy= %f\n', layerIdx, acc15);
            ...
            for i = 1 : maxIter
                ...
                fprintf('Layer %d: training acc = %f, testing acc = %f\n', layerIdx, acc_train, acc_test);
                trainACC(i, layerIdx) = acc_train;
                testACC(i, layerIdx) = acc_test;
                ...
                % convergence check
                Xi = 1 - Y_train.*scores_train;
                Xi(Xi<0)=0;
                seperation = (W(1:end-1)' * W(1:end-1)) / 2;
                violation = C * sum(Xi);
                fprintf('seperation = %f, violation = %f\n', seperation, violation);
                obj = seperation + violation;

                fprintf('%dth iter: objVal = %f, deltaObj=%f\n', i, obj, abs(lastObj - obj));

                if abs(lastObj - obj) / obj < optObj
                    break
                end
                lastObj = obj;
                
                % update M
                fprintf('Start optimize M: ');
                temp = zeros(trainNum, length(M));
                grads = zeros(trainNum, length(M));
                for k = 1:trainNum
                    X_curr = X_train_raw(:,:,k);
                    temp(k,:) = W(1:end-1)' * X_curr';
                    grads(k,:) = Y_train(k) * temp(k,:);
                end
                tempObj = Inf;
                for j = 1 : maxIter
                    fprintf('#');
                    vals = Y_train .* (temp * M - W(end));
                    
                    % calculate objective value
                    Xi = 1 - vals;
                    Xi(Xi<0)=0;
                    obj = (W(1:end-1)' * W(1:end-1)) / 2 + C * sum(Xi);
                    fprintf('obj = %f', obj);
                    
                    if abs(tempObj - obj) / obj < optObj
                        break
                    end
                    tempObj = obj;
...
%                     tempM = M;
    ...
    rst15 = computeDSR (data_test, W15_all, [], 'feat15')
%     rstCMI = computeDSR (data_test, W_all, M_all, 'CMI')
...
function [X, Y] = extractFeatureBF (data, gts, mapping)
    if nargin < 3
        mapping = [];
    end
...
function [ acc, weights ] = evaluateSVMFeat15 ( X_train_raw, Y_train, X_test_raw, Y_test )
...
function [ rst ] = computeDSR (test_set, Ws, Ms, modelString)
    ...
    switch modelString
        ...
        case 'feat15'
            ...
            elseif size(Ws,1) == 51
                mapping = 'quadratic';
            elseif size(Ws,1) == 101
                mapping = 'polynomial';
            end
            
            for j = 1 : size(test_set,1)
                dimension = size(test_set{j,1},1);
                [test_X, ~] = extractFeatureBF(test_set(j,1), [], mapping);
                test_X(:,size(test_X,2)+1) = -1;
                recImg = zeros(dimension, dimension, 3);
                for layerIdx = 1:3
                    W = Ws(:,layerIdx);
        ...
        case 'CMI' % CMI model
            ...
            for j = 1 : size(test_set,1)
                dimension = size(test_set{j,1},1);
                recImg = zeros(dimension, dimension, 3);
                [X_test_raw, ~] = extractRawFeature(test_set(j,1), [], mapping);
                testNum = size(X_test_raw, 3);
                test_X = -ones(testNum, size(Ws,1));
                for layerIdx = 1:3
                    M = Ms(:,layerIdx);
                    for k = 1 : testNum
                        test_X(k, 1:end-1) = M' * X_test_raw(:,:,k);
                    end
                    W = Ws(:,layerIdx);
                    scores = test_X * W;
                    pred = double(scores > 0);
