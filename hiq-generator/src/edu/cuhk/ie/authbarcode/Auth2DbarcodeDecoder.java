...
//Global variables
	...
	public Auth2DbarcodeDecoder(PrivateKey signer, X509Certificate Cert){
		...
		if(signer==null||Cert==null) Log("Invalid key or certificate");
...
//Part 1: data handling functions
	...
	public Object getDataById(String id){
		...
		try {
			...
			Log("Cannot return data value. Detail:"+e);
		}
		return null;
	}
	public String getFormatById(String id){
		...
		try {
			...
			Log("Cannot return content type. Detail:"+e);
		}
		return null;
	}
...
//Check if the message format is supported
	public static Auth2DbarcodeDecoder getSupportedDecoderByte(byte[] encodedArray){
		...
		Log("The message size is:"+encodedArray.length);
		byte[] temp=null;
	...
	protected static byte[] defaultCompress(byte[] message){
		...
		if(baos.size() <= 1) {
			Log("Message is created but not compressed.");
			return null;
		}
...
//Part 4: decoding the Authenticated 2D barcode according to its type
	...
	public static boolean setDefaultCert(KeyStore trustStore, InputStream defaultCert){
		if(trustStore ==null || defaultCert ==null) return false;
		try{
			CertificateFactory certFactory = CertificateFactory.getInstance("X.509");
			X509Certificate cert = (X509Certificate) certFactory.generateCertificate(defaultCert);
			trustStore.setCertificateEntry(signerCert, cert);
			return true;
	...
	private static X509Certificate getCertificate(KeyStore store, String subjectDN){
		...
		try {
	        ...
	        //If failed, return the default certificate
	        Log("Fail to get a certificate, use the default one");
	...
	protected X509Certificate getVerifiedCertificate(byte[] embededCert, KeyStore trustStore, String subjectDN){
		...
		try {
			...
			String issuerDN="";
			try{
				issuerDN= (cert !=null)? getCertificateSubjectCN(cert):subjectDN;
			}catch(Exception e3){
	...
	public static String getCertificateSubjectCN(X509Certificate cert){
		...
		String[] endFields={",OU=", ",O=", ",L=", ",ST="};
		for(int i=0, endIndex=-1;i<endFields.length;i++){
			endIndex = subjectDN.lastIndexOf(endFields[i],searchLength);
			if(endIndex>startIndex) return subjectDN.substring(startIndex+startField.length(), endIndex);
		}
	...
	protected static boolean verifySignature(byte[] messageToSign, byte[] signature, String signatureMethod, X509Certificate issuerCert){
		...
		try{
			...
			if(signatureAgent==null) signatureAgent=Signature.getInstance(signatureMethod);
